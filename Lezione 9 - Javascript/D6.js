...
let Paolo = 'Palombella'
let Peterson = 'Patata'

function conc(str1, str2){
  let array = [str1.slice(0,2), ...str2.slice(- 3, str2.length)]; 
  let c = array.join ('');
...
let result = num.filter((pat) => pat%2===0)
...
//riutilizzo il num creato nell'esercizio 3

let s=0;
num.forEach(pat =>{s += pat;})
console.log(s)
...
console.log(num.reduce((previousValue, currentValue) => previousValue + currentValue, 0))
...
//provare a vedere come fare se sono due dello stesso anno e poi ne torna solamente uno vecchi
...
let sumYear = (arr) => {
  return arr.reduce((accumulatore, valoreCorrente) => accumulatore + Number(valoreCorrente.film), 0)
}
